...
void
Vram::Save(const std::string& file)
{
...
Vram::PutU8(u16 x, u16 y, u8 byte)
{
    ...
    {
        LOGGER->Log("Vram::PutU8: Error: Tried to put byte in Vram x(" + std::to_string(x) + ") y(" + std::to_string(y) + ")\n");
        return;
    }
...
Vram::GetU8(u16 x, u16 y) const
{
    if (x > m_Width || y > m_Height)
    {
        LOGGER->Log("Vram::GetU8: Error: Tried to get byte from Vram x(" + std::to_string(x) + ") y(" + std::to_string(y) + ")\n");
        return 0;
    }
...
Vram::PutU16(u16 x, u16 y, u16 bytes)
{
    ...
    {
        LOGGER->Log("Vram::PutU16: Error: Tried to put pixel in Vram x(" + std::to_string(x) + ") y(" + std::to_string(y) + ")\n");
        return;
    }
...
Vram::GetU16(u16 x, u16 y) const
{
    if (x + 1 > m_Width || y > m_Height)
    {
        LOGGER->Log("Vram::GetU16: Error: Tried to get pixel from Vram x(" + std::to_string(x) + ") y(" + std::to_string(y) + ")\n");
        return 0;
    }
